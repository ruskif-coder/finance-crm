...
/** Размеры плиток главного виджета — ОДНИ на дашборд и на «Кампании».

    Значения из карточки хендоффа (экран «Кампании»): её рисовал дизайнер, а плитки
    дашборда восстанавливались руками и кегли брались на глаз — виджет «прыгал» при
    переходе между разделами (владелец 15.09.2026).

    Живут В КИТЕ, а не в одном из экранов: импорт размеров из соседнего компонента
    замкнул бы `Dashboard` и `CampaignsScreen` друг на друга, а цикл в модулях даёт
    `undefined` в момент вычисления — поломку, которая проявляется не там, где сделана. */
export const KPI_SIZE = { label: 10, value: 26, unit: 11.5, hint: 11.5 }
