-- «Актуальные кампании» в кабинете паблишера: витрина размещений.
--
-- Блок возвращается в кабинет по хендоффу `docs/актуальные кампании.zip` (владелец
-- 15.09.2026) и на этот раз собирается из НАСТОЯЩИХ данных: до сих пор он жил в
-- `cabinet-frontend/lib/demo.js` под флагом `SHOW_MONEY = false`, то есть был выключенной
-- выдумкой.
--
-- ЕДИНИЦА СТРОКИ — РАЗМЕЩЕНИЕ, пара «сделка × площадка» (`launch_prep_target`). Один
-- бренд встречается несколько раз — августовский и сентябрьский флайты это разные строки
-- со своим фактом, биллингом и ЕРИД. Группировки по услуге и месяцу в блоке нет.
--
-- ПЕРИОД НЕ ХРАНИТСЯ ОТДЕЛЬНЫМ ПОЛЕМ: экран выводит его из месяца флайта. Требование
-- хендоффа, и оно про реальный дефект — пара «период ↔ срок РК» расходилась, декабрьский
-- флайт лежал в сентябрьском периоде. Здесь отдаются даты, а месяц считает экран.
