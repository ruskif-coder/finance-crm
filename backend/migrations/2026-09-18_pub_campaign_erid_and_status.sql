-- ЕРИД и статус в кабинете площадки: читать оттуда, где они на самом деле лежат.
--
-- Площадка видела свою кампанию (порог ответа из `2026-09-17` работает), но в строке
-- стояло «ЕРИД не выпущен» и «ждёт согласования» — при выпущенном маркере и её же
-- согласовании. Две разные ошибки с одним корнем: витрина спрашивала не тот источник.
--
-- 1. МАРКЕР. Лямбда искала комплект по `launch_prep_creative_set.publisher_id`, а он
--    NULL: комплект принадлежит СДЕЛКЕ, с площадкой его связывает пара
--    (`launch_prep_pair.target_id`). Колонка с читателем и без писателя — та же болезнь,
--    что с `ad_campaign_creative.erid` накануне. Теперь маркер берётся через пару этой
--    строки, а старое условие оставлено запасным путём для комплектов, заведённых с
--    площадкой напрямую.
--
-- 2. СТАТУС. В лестнице не было НИ ОДНОЙ ветки после «согласован»: «ерид получен»,
--    «заведён в DSP» и «в размещении» проваливались в ELSE, то есть в «ждёт
--    согласования». Площадка, уже согласовавшая креатив и получившая маркер, читала,
--    что от неё всё ещё ждут согласования.
--
-- Порядок ветвей = приоритет, и он не переставлен: отказ старше всего, дальше факт из
-- контура трафика (площадка в РК запущена — это сильнее, чем статус всей кампании),
-- потом даты, и только потом состояние пары.
--
