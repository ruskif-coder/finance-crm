...
def test_a_nameless_extra_cannot_be_saved():
    """Доп. услуга без названия не сохраняется.

    Название выбирается из справочника, свободного ввода нет — значит пустое имя
    означает ровно одно: строку добавили и бросили. В конструкторе она видна подсказкой
    «выберите доп. услугу», а в ВЫГРУЖЕННОМ документе превращается в пустой ряд, и
    клиент читает его как ошибку.

    Замер на проде 15.09.2026: из 66 строк доп. услуг безымянная ровно одна — сделка
    WGUPN5, из-за неё и была жалоба «МП всё ещё без доп. услуг». То есть дефект не в
    коде выгрузки, а в том, что незаполненную строку вообще пустили в базу.

    Прибор смотрит на ИСХОДНИК: поднять реальный POST здесь значит втащить право,
    own-scope и сессию запроса, а вопрос ровно один — есть ли отказ и стоит ли он ДО
    записи.
    """
    import inspect
    from app.routers import media_plans as mp
    src = inspect.getsource(mp.save_media_plan)
    assert "без названия" in src, "пустое имя доп. услуги проходит в базу"
    assert src.index("без названия") < src.index("_write_children"), (
        "проверка стоит после записи — строка уже в базе")
