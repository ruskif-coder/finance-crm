...
ALLOWED_DESTRUCTIVE = (
    # Возврат умолчания, а не разрушение: в `cabinet_account_notify` строка ОЗНАЧАЕТ
    # отклонение от умолчания, её отсутствие — «канал включён». Включить вид обратно
    # можно только удалив строку, и удаляется ровно одна, по первичному ключу.
    # (До 15.09.2026 здесь стояла `cabinet_account_mute` — та же роль без измерения
    # каналов; таблица заморожена, читателей у неё не осталось.)
    "DELETE FROM cabinet_account_notify WHERE account_id = :a AND kind = :k AND channel = :c",
    # «Вернуть по умолчанию» — кнопка под матрицей. Сносит ВСЕ отклонения одной учётки,
    # и это её обещание целиком: умолчание не состояние, а отсутствие решения, поэтому
    # «вернуть» и означает «убрать решения». Область — одна учётка, чужого не касается.
    "DELETE FROM cabinet_account_notify WHERE account_id = :a",
