...
def test_status_vocabulary_is_pinned():
    ...
    (владелец 04.09.2026).
    """
    # «Ждёт сборки» добавлено 18.09.2026 в начало конвейерной части: площадка попадает
    # в РК кандидатом, когда креативов по ней ещё нет, и называть это «у трафика» значит
    # звать человека к несуществующей задаче.
    assert PLACEMENT_STATUSES == ('ждёт сборки', 'у трафика', 'у площадки', 'ждёт запуска',
...
def test_placement_state_is_the_best_of_its_creatives():
    ...
    # Пустой список — это НЕ «мяч у трафика», а «сборка ещё идёт» (владелец 18.09.2026).
    assert best_chain_status([]) == 'ждёт сборки', 'креативов нет — сборка не закончена'
