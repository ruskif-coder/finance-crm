...
class _C:
    def __init__(self):
        ...
        # Маркер обязателен с 17.09.2026: креатив без ЕРИД в сеть не уходит, и проверка
        # на него стоит РАНЬШЕ пикселя. Без этого поля прибор проверял бы отказ по
        # пикселю на креативе, который до пикселя и не дошёл бы.
        self.erid = "Kra23r5Lz"
