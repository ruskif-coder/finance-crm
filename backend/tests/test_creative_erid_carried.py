...
def test_the_archive_is_carried_with_the_marker():
    ...
    Берём архив согласованного комплекта, а не корня цепочки: корень мог быть отвергнут и
    заменён, и в сеть уехал бы не тот баннер — тот же довод, что у маркера.
    """
    import inspect
    ...
    assert 'archives.get(r["set_id"])' in src, (
        "файл берётся не с того комплекта: нужен комплект ПАРЫ, а не корень цепочки")
...
def test_the_creative_number_is_the_number_people_see():
    ...
    Номер берётся у КОРНЯ цепочки: доработка рождает новый комплект со своим номером, а
    креатив остаётся тем же рекламным сообщением. И перенумеровывается только то, что ещё
    не уехало в DSP — имя заведённого креатива там уже зафиксировано, переименовать его
    нечем, и расхождение «у нас cr4, в кабинете cr1» было бы хуже исходной путаницы.
    """
    import inspect
    ...
    assert "set_no.get(root)" in src, "номер обязан браться у корня цепочки"
