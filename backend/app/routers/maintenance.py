...
def maintenance_state(db: Session = Depends(get_db),
    ...
    """Состояние + ОТВЕТ НА ВОПРОС «а меня-то пускает?».

    Поле `passes` появилось 18.09.2026 после поломки, в которой виноват был не заслон, а
    экран. Прослойка админа пропускала честно — а `MaintenanceStub` рисовался всем, у
    кого режим активен, роли он не спрашивал. Получилось ровно то, чего модуль клялся не
    допустить: включивший обслуживание закрыл портал себе же, и снимать режим пришлось
    из консоли базы.

    Теперь решает ОДНО место — сервер, который и так решает, пускать ли запрос. Экрану
    остаётся подчиниться, а не считать заново: два расчёта одного права разошлись бы
    молча, и разошлись.
    """
    st = mnt.state(db)
    st["passes"] = bool(getattr(user, "role", None)
                        and getattr(user.role, "key", None) == "admin")
    return st
