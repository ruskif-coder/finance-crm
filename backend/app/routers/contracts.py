...
def _serialize(c: Contract) -> dict:
    return {
        ...
        # Отметка ОРД. Ставит её загрузка выгрузки и ручная привязка на экране ОРД —
        # здесь ТОЛЬКО ЧИТАЕТСЯ: второй писатель на то же поле развёл бы значения.
        # Контур обязателен рядом с идентификатором: демо и прод дают РАЗНЫЕ id одному
        # договору, и демовский снаружи неотличим от боевого.
