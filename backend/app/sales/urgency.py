...
def evaluate(f: DealFacts, today: date) -> Verdict:
    ...
    # 1a. План есть, а человек его ещё не смотрел. Сделка стоит на первой стадии:
    # отметка «Проверено» в конструкторе сама двинет её дальше. Это не «ждём клиента»
    # (mp_unapproved) — тут мяч у аккаунта, и дата старта не важна: непроверенный план
    # нельзя отправлять клиенту в любом случае.
    #
    # Формулировка НЕ называет конвейер: путей рождения плана и сделки два и они
    # равноправные — пачкой из годового плана и вручную из малого МП (кнопка «Создать
    # сделку» в конструкторе). Текст «собран конвейером» на ручном плане был неправдой.
