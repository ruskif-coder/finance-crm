...
class DealFacts:
    ...
    # Фактов `mp_approved`/`mp_rejected` здесь больше нет (30.08.2026). Они читались со
    # статуса медиаплана, а своего состояния у плана не осталось: где план — говорит
    # стадия сделки. «Отдан клиенту» = `stage_key == "media_plan" and not stage_is_first`,
    # «принят» = сделка ушла на «Бронь», «забракован» = сделку вернули назад, и тогда она
    # снова первая по лестнице и её поднимает правило 1a.
...
def evaluate(f: DealFacts, today: date) -> Verdict:
    ...
    # 1. Нет медиаплана. Работа для аккаунта, а не срочность: сделка заведена, плана нет —
    # значит считать её кому-то придётся, и увидеть это надо СРАЗУ, а не за пять дней до
    # старта. До 15.09.2026 правило требовало `start_in <= 5`, и новая сделка со стартом
    # через месяц не попадала в «Нужен МП» вовсе — владелец так и сообщил: «создали
    # сделку, у аккаунта не появилась». Замер в день правки: на предстартовых стадиях без
    # плана 185 сделок, в очередь попадали 38.
    #
    # Срочность при этом остаётся честной — она тут и есть мера близости старта:
    # OVERDUE ближе пяти дней, SOON до двух недель, дальше NORMAL («в работе, не горит»).
    # Лента уведомлений NORMAL не рассылает (см. scanner._deal_rule), так что расширение
    # очереди не превращается в рассылку по каждой будущей сделке.
    #
    # Окна у правила нет ВООБЩЕ — ни вперёд, ни назад (владелец 15.09.2026: «статус
    # „нужен МП" висит всегда в активных, ибо сейлзы могут накидывать работу на год
    # вперёд, а считать надо сейчас»). Отсечка забытого старта (STALE_START_DAYS)
    # к этому правилу не применяется: сделка без плана — это несделанная работа, а не
    # устаревшая новость, и «старт был давно» её не отменяет, а усугубляет.
    # Дат может не быть вовсе: у сделки без старта план нужен тем более — считать её
    # никто не начнёт, пока она не попадётся на глаза.
    if f.stage_key in PRE_LAUNCH_KEYS and not f.has_mp:
        if start_in is None:
            return Verdict(NORMAL, "Даты размещения не заданы, МП не готов",
                           "Собрать МП", "deal_mp_missing")
        return Verdict(OVERDUE if start_in <= 5 else
                       (SOON if start_in <= BOOKING_CONFIRM_DAYS else NORMAL),
                       f"{start_phrase()}, МП не готов",
                       "Собрать МП", "deal_mp_missing", f.period_from)

    # 1a. «План есть, а человек его ещё не смотрел» (вердикт `mp_verify`) УБРАНО
    # 13.09.2026. Правило опиралось на состояние «сделка на первой стадии, но план у неё
    # уже есть» — а с этого дня прикрепление плана само уводит сделку на «МП Отправлено»
    # (`_advance_deal_on_link`), и такого состояния не возникает. Замер на день удаления:
    # 0 сделок из 920. Промежуточной ступени между «собираем» и «отправлено» нет
    # (владелец 13.09.2026), значит и «непроверенного плана» как состояния нет тоже.

    # 2. План отдан клиенту, ответа нет, а старт совсем близко. «Отдан» читается со
    # СТАДИИ: сделка на второй стадии песочницы («МП Отправлено»). Раньше признаком был
    # статус плана `approved is False`, и он означал не то, что написан: статус ставил
    # наш согласующий, а ждут здесь клиента.
    if (pre_launch and f.stage_key == "media_plan" and not f.stage_is_first
            and start_in <= 3):
        return Verdict(OVERDUE, f"{start_phrase()}, МП у клиента без ответа",
