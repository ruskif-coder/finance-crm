...
УДАЛЕНО 30.08.2026: app/sales/bitrix_client.py — классический вебхук портала, заменённый
VibeCode. Год числился здесь как «легаси, не использовать», но при этом лежал рядом с
живым кодом, имел самое очевидное из трёх имён и девять зелёных тестов — то есть
выглядел поддерживаемым. Пометки в карте слоёв оказалось мало: нашёл его граф связности,
а не чтение. Уцелел только payload_hash — он про правило хранения, а не про транспорт,
и переехал в app/sales/sync.py вместе с двумя своими тестами.
