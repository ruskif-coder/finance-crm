...
SECTIONS = [
    ...
    # Экран «Заполнение» — тот же набор действий, что и в карточке, но пачкой и без
    # подтверждения: каждый клик уходит на сервер сразу. Поэтому право на него отдельное
    # и ТОЛЬКО на просмотр: сама запись по-прежнему требует "изменение" в «Площадках»
    # (bulk ходит в те же эндпоинты, что и карточка). Разделение отвечает на вопрос
    # «кому показывать быстрый экран», а не «кому можно править».
    {"key": "dir_publishers_bulk", "label": "Паблишеры · Заполнение", "group": "Паблишеры", "actions": ["view"]},
