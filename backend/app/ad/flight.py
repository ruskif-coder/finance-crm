...
PLACEMENT_CHAIN = ("у трафика", "у площадки", "ждёт запуска")
...
PLACEMENT_NEW = PLACEMENT_CHAIN[0]        # «у трафика» — мяч у нас
PLACEMENT_AT_SITE = PLACEMENT_CHAIN[1]    # «у площадки» — ждём её вердикта
PLACEMENT_READY = PLACEMENT_CHAIN[2]      # «ждёт запуска» — согласована, не включена
...
def creative_counts(creatives):
    ...
    return {
        "total": len(creatives),
        "agreed": sum(1 for c in creatives
                      if c.get("status") not in ("у трафика", "у площадки")),
