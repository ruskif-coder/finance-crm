...
class AdCampaignCreative(Base):
    ...
    Зерно — КОМПЛЕКТ на площадке. Доработка — новый порядковый креатив со своим номером,
    маркером и файлом (владелец 23.09.2026: «правки не перезаписываются, а уходят в новый
    порядковый»). До этого (04.09.2026) зерном было рекламное сообщение — корень цепочки
    доработок, и правка подменяла файл и ЕРИД у строки, уже уехавшей в DSP, где оставался
    старый баннер. Связи, добавленные миграцией `2026-09-04_ad_creative_links.sql`:

      · `root_set_id` — КОМПЛЕКТ строки. Имя осталось от прежнего правила, когда здесь
        лежал корень цепочки; по нему же строится обратный вид «креатив → площадки»;
      · `pair_id` — пара согласования этого комплекта на этой площадке.

    Уникальность `(placement_id, root_set_id)`: один комплект на одной площадке — одна
    строка.
    """
