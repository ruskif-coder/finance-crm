...
class LaunchPrepCreativeSet(Base):
    ...
    # Креатив НАЦЕЛИВАНИЯ в DSP (миграция `2026-09-12_targeting_creative.sql`): заводится
    # у демоклиента на отправке трафику, и на него выпускается ссылка, привязывающая
    # браузер трафика. Не путать ни с боевым креативом (свой хеш в `ad_campaign_creative`,
    # появляется только после согласования площадки), ни с ПРЕДПРОСМОТРОМ: тот читает тот
    # же архив, но живёт у нас в песочнице и наружу ничего не отправляет.
    #
    # Хеш хранится потому, что в DSP нет удаления: повторная отправка не должна заводить
    # второй такой же креатив, а другого признака «уже заведён» не существует.
    ms_targeting_creative_xxhash = Column(String(32))
    ms_targeting_at = Column(DateTime)
