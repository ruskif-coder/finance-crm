...
register(Event(
    ...
    # Обе роли: рядовой трафик берёт работу, мастер видит поток. Резолвера
    # «ответственный трафик» нет намеренно — `traffic_manager_id` пуст у всех сделок,
    # и он вернул бы никого.
    recipients=[{"type": "role", "value": "role_120"},
                {"type": "role", "value": "role_121"}],
    channels={"app": True},
))
...
register(Event(
    ...
    # Мастер траффик ролью: `traffic_manager_id` пуст у всех сделок, назначений ещё нет,
    # и резолвер «ответственный трафик» сегодня вернул бы никого. Аккаунт — потому что
    # это ЕГО сделка стоит, и узнать он должен не от клиента.
    recipients=[{"type": "role", "value": "role_121"},
