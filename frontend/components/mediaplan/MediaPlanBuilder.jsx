...
export default function MediaPlanBuilder({ brief, catalog = CATALOG, extraCatalog = EXTRA_CATALOG, staff = STAFF, models = MODELS, modes = MODES,
  ...
  const params = [
    ['Рекламодатель', brief?.advertiser ?? 'Woerwag Pharma'], ['Бренд', brief?.brand ?? 'Мильгамма'],
    ['Агентство', brief?.agency ?? 'RX Code'], ['Контрагент (Юр.лицо)', brief?.payer ?? 'RX Code'],
    ['Период размещения', brief?.period ?? 'месяц · 2026-09'], ['Гео', brief?.geo ?? 'РФ'],
  ];
  const targeting = brief?.targeting ?? [
    ['Аудитория', 'Ж/М 30–60 · гео РФ'],
    ['Покупают', 'витамины группы B · препараты при нейропатии · обезболивающие при болях в спине'],
    ['Интересы', 'неврология · здоровье спины и суставов · медицина'],
    ['Поведение', 'сайты аптек · медицинские порталы · запись к неврологу'],
    ['Конкуренты', 'Комбилипен · Нейромультивит · Нейробион · Тиогамма'],
  ];
  const GOALS = [
