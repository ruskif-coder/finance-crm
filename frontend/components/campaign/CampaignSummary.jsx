...
const GRID = 'minmax(0,1.4fr) 52px 104px 62px 104px 104px 104px 120px'
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ paddingTop: 12, borderTop: '1px solid var(--border-inner)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: GRID, gap: 9,
            ...
            {['Площадка', 'Код', 'Вес', 'Доля', 'План', 'Факт', 'Недокрут', 'Статус']
              .map((h, i) => (
                <span key={h} style={{ ...CAP, textAlign: i >= 2 && i <= 6 ? 'right' : 'left' }}>
                  {h}</span>
              ))}
          </div>
          {(d.rows || []).map(r => (
