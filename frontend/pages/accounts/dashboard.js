...
const GROUPS = [
  ...
  // Отдельная группа: план уже есть, но его собрал конвейер и человек не смотрел.
  // Не «нужен МП» (он есть) и не «ждём клиента» (ему ещё нечего показывать).
  { key: 'mp_verify', title: 'МП завизировать', hint: 'собран конвейером, аккаунт не открывал',
    dot: 'var(--dot-current-dz)', kinds: ['mp_verify'] },
  { key: 'mp_sent', title: 'МП отправлен', hint: 'ждём визу клиента или переделываем после отказа',
    dot: 'var(--dot-current-dz)', kinds: ['mp_unapproved', 'mp_rework'] },
...
export default function AccountDashboard() {
  ...
  // «Рекламодатель = BINNO», человек видел бы в очереди 12 строк, а в виджете рядом
