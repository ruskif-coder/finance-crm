    ...
    return (
      <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '16px 20px', flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', marginBottom: '6px' }}>
          <span style={{ fontSize: '22px', fontWeight: '700', color: '#111827' }}>{fmt(fact)} ₽</span>
          <span style={{ fontSize: '13px', fontWeight: '600', color }}>{fmtPct(pct)}</span>
        </div>
    ...
    <div style={{ minHeight: '100vh', background: '#f5f6fa' }}>
      ...
      <div style={{ padding: '20px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: '14px', marginBottom: '16px' }}>
          ...
          <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '16px 20px', flex: 1 }}>
            <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '8px' }}>Чистая прибыль</div>
            <div style={{ fontSize: '22px', fontWeight: '700', color: summary.fact_net >= 0 ? '#15803d' : '#dc2626', marginBottom: '6px' }}>
              {fmt(summary.fact_net)} ₽
            </div>
            <div style={{ fontSize: '12px', color: '#6b7280' }}>План: {fmt(summary.plan_net)} ₽</div>
          </div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '20px', marginBottom: '16px' }}>
          <div style={{ fontWeight: '500', marginBottom: '16px', fontSize: '14px' }}>Выполнение плана по месяцам</div>
        ...
        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e5e7eb', overflow: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr>
                <th style={{ padding: '8px 16px', fontSize: '12px', fontWeight: '500', color: '#6b7280', textAlign: 'left', background: '#f9fafb', borderBottom: '2px solid #e5e7eb' }}>Статья</th>
                {['План', 'Факт', 'Отклонение', '% выполнения'].map(h => (
                  <th key={h} style={{ padding: '8px 12px', fontSize: '12px', fontWeight: '500', color: '#6b7280', textAlign: 'right', background: '#f9fafb', borderBottom: '2px solid #e5e7eb', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                        ...
                        <>
                          {hasSubgroup && (
                            <tr key={sgKey} onClick={() => toggleGroup(sgKey)}
                              ...
                              <td style={{ padding: '7px 16px 7px 20px', fontWeight: '500', fontSize: '12px', color: '#374151', whiteSpace: 'nowrap' }}>
                                {expandedGroups[sgKey] !== false ? '▾' : '▸'} {sg.subgroup}
                              </td>
                              <td style={{ padding: '7px 12px', textAlign: 'right', fontSize: '12px', fontWeight: '500', color: '#374151' }}>{fmt(sg.totals.plan)} ₽</td>
                              <td style={{ padding: '7px 12px', textAlign: 'right', fontSize: '12px', fontWeight: '500', color: '#374151' }}>{fmt(sg.totals.fact)} ₽</td>
                              <td style={{ padding: '7px 12px', textAlign: 'right', fontSize: '12px', fontWeight: '500', color: sg.totals.diff >= 0 ? '#15803d' : '#dc2626' }}>
                                {sg.totals.diff >= 0 ? '+' : ''}{fmt(sg.totals.diff)} ₽
                              </td>
                              <td style={{ padding: '7px 12px', textAlign: 'right', fontSize: '12px', fontWeight: '600', color: pctColor(sg.totals.pct, g.is_income) }}>
                          ...
                          {(expandedGroups[sgKey] !== false) && sg.articles.map(a => (
                            <tr key={a.article} style={{ borderBottom: '1px solid #f3f4f6' }}
                              ...
                              <td style={{ padding: '6px 16px 6px ' + (hasSubgroup ? '36px' : '28px'), color: '#6b7280', fontSize: '12px', whiteSpace: 'nowrap' }}>
                                {a.article}
                              </td>
                              <td style={{ padding: '6px 12px', textAlign: 'right', fontSize: '12px', color: '#374151' }}>{a.plan > 0 ? fmt(a.plan) + ' ₽' : '—'}</td>
                              <td style={{ padding: '6px 12px', textAlign: 'right', fontSize: '12px', color: '#374151' }}>{a.fact > 0 ? fmt(a.fact) + ' ₽' : '—'}</td>
                              <td style={{ padding: '6px 12px', textAlign: 'right', fontSize: '12px', color: a.diff >= 0 ? '#15803d' : '#dc2626' }}>
                                {(a.plan || a.fact) ? (a.diff >= 0 ? '+' : '') + fmt(a.diff) + ' ₽' : '—'}
                              </td>
                              <td style={{ padding: '6px 12px', textAlign: 'right', fontSize: '12px', fontWeight: '500', color: pctColor(a.pct, g.is_income) }}>
                                {fmtPct(a.pct)}
                              </td>
                            </tr>
                          ))}
