...
export default function TrafficDashboard() {
  ...
  return (
    <>
      <Head><title>Дашборд трафика</title></Head>
      <Navbar />
      <div style={{ maxWidth: 2500, margin: '0 auto', padding: '18px 28px 60px', fontFamily: UI }}>
        ...
        <div style={{ ...card, padding: '16px 18px' }}>
          ...
          {rows.map(r => {
            ...
            const d = detail[r.id]
            // Тон строки — из общего кита, а не свой: `today` (кремовая заливка + рамка)
            // означает «требует твоего шага сегодня», и нераспределённая РК — ровно это.
            const u = r.traffic_rep_id ? ROW_TONE.normal : ROW_TONE.today
            return (
              <div key={r.id}>
                {/* Строка — ОБВЕДЁННАЯ карточка, тонами из общего кита `ROW_TONE`: ровно
                    ...
                    Нераспределённая РК берёт тон `today` (кремовая заливка + рамка):
                    отсутствие ответственного — свойство СТРОКИ, а не одной ячейки, и
                    столбца под него нет. Заливка сама по себе почти белая — читается
                    именно рамка, поэтому подсветка живёт парой, а не одним фоном.
