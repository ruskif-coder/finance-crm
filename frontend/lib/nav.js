...
// Здесь стояла `legacyRedirects()` — то же преобразование `data.redirects` в формат
// Next, «оставленное для кода приложения». Потребитель за всё время не появился: конфиг
// читает `nav.data.json` напрямую (иначе никак — он CommonJS, а этот модуль ESM и тянет
// ./auth), а клиентский двойник разбирает правила сам через `lib/legacy.mjs`. Снято
// 30.08.2026: вторая живая реализация одного и того же — это то, что однажды разойдётся,
// а мёртвая ещё и приглашает ею воспользоваться.
